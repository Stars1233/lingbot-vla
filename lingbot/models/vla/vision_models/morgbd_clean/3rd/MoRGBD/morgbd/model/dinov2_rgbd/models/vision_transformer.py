...
class DinoVisionTransformer(nn.Module):
    ...
    def prepare_tokens_with_masks(self, x, masks=None, depth_sample_step=4):
        assert masks is None # not support masks for now
        B, nc, h, w = x.shape
        mae_mask, mae_ids_restore = None, None

        assert nc == 5 # [img, depth, depth_mask]
        img = x[:, :3]
        depth = x[:, 3:4]
        x_img = self.patch_embed(img)

        if self.depth_emb_mode == 'conv_1c':
            x_depth = self.depth_mask_patch_embed(depth)
        else:
            raise NotImplementedError
        
        mask_ratio = self.mask_ratio
        img_mask_ratio = self.img_mask_ratio
        if self.img_depth_fuse_mode == 'cat_token':
            N, L, _ = x_depth.shape  # batch, length, dim
            assert N == B

            ## process depth tokens
            depth_pose_enc = self.interpolate_pos_encoding_without_cls(x_depth, h, w, self.pos_embed[:, 1:]).repeat(N, 1, 1)
            assert depth_pose_enc.shape[0] == N
            
            if (mask_ratio == 0) or (N == 1 and not self.training):
                x_depth_masked = x_depth[:,::depth_sample_step]
                ...
                x_depth_masked = torch.gather(x_depth, dim=1, index=ids_keep.unsqueeze(-1).repeat(1, 1, x_depth.shape[-1]))
    ...
    def get_intermediate_layers_mae(
        ...
        if self.chunked_blocks:
            outputs, mae_mask, mae_ids_restore = self._get_intermediate_layers_chunked(x, n, return_mae_aux=return_mae_aux)
        else:
            outputs, mae_mask, mae_ids_restore = self._get_intermediate_layers_not_chunked(x, n, return_mae_aux=return_mae_aux)
        if norm:

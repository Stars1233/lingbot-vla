...
def mask_bce_loss(pred_mask_prob: torch.Tensor, gt_mask_pos: torch.Tensor, gt_mask_neg: torch.Tensor) -> torch.Tensor:
    ...
    eps = 1e-6
    pred_mask_prob = torch.clamp(pred_mask_prob, eps, 1 - eps)

    loss = (gt_mask_pos | gt_mask_neg) * F.binary_cross_entropy(pred_mask_prob, gt_mask_pos.float(), reduction='none')
    loss = loss.mean(dim=(-2, -1))
    return loss, {}

def mask_bce_with_logits_loss(pred_mask_logits: torch.Tensor, gt_mask_pos: torch.Tensor, gt_mask_neg: torch.Tensor) -> torch.Tensor:
    valid_mask = gt_mask_pos | gt_mask_neg
    loss = F.binary_cross_entropy_with_logits(pred_mask_logits, gt_mask_pos.float(), reduction='none') * valid_mask.float()

    loss = loss.sum(dim=(-2, -1)) / (valid_mask.sum(dim=(-2, -1)) + 1e-6)
...
def depth_l1_loss(pred_depth, gt_depth, mask=None, max_depth=None):
    pred_depth = pred_depth.squeeze()
    gt_depth = gt_depth.squeeze()

    invalid_depth_mask = torch.isfinite(gt_depth) & (gt_depth < 0.1) & torch.isnan(gt_depth)
    valid_depth_mask = ~invalid_depth_mask
    if mask is not None:
        valid_depth_mask &= mask
    if max_depth is not None:
        valid_depth_mask &= (gt_depth < max_depth)
    if valid_depth_mask.sum() == 0:
        return torch.tensor(0.0, device=pred_depth.device), {}
    
    loss = (pred_depth[valid_depth_mask] - gt_depth[valid_depth_mask]).abs().mean()
    return loss, {}
...
from morgbd.train.dataloader import TrainDataLoaderPipeline
from morgbd.train.losses import (
    ...
    normal_map_loss,
    monitoring, 
    mask_bce_with_logits_loss,
    depth_l1_loss
)
from morgbd.train.utils import build_optimizer, build_lr_scheduler
from morgbd.utils.geometry_torch import intrinsics_to_fov
from morgbd.utils.vis import colorize_depth, colorize_normal
from morgbd.utils.tools import key_average, recursive_replace, CallbackOnException, flatten_nested_dict
...
def main(
    ...
    if not torch.cuda.is_bf16_supported():
        print("The current GPU does not support bfloat16")
        return
    ...
    # Set logger
    logger = setup_logger(f'morgbd-rank{accelerator.process_index}/{accelerator.num_processes}', workspace, out_file='train.log', json_format=True, rank=accelerator.process_index)    
    ...
    with accelerator.local_main_process_first():
        from morgbd.model import import_model_class_by_version
        MoRGBDModel = import_model_class_by_version(config['model_version'])      
        model = MoRGBDModel(**config['model'])
    ...
    with accelerator.local_main_process_first():
        if checkpoint_path is None:
            ...
            if checkpoint_path.exists():
                ...
                logger.info(f'No latest checkpoint found. Start from scratch.')
                checkpoint = None
        else:
            # - Load by step number
    ...
    if torch.version.hip and isinstance(model, torch.nn.parallel.DistributedDataParallel):
        ...
        from morgbd.model.utils import sync_ddp_hook
    ...
    with accelerator.local_main_process_first():
        ...
        # train_data_pipe = TrainDataLoaderPipeline(config['data'], batch_size_forward, num_load_workers=1, num_process_workers=1)
    ...
    def _write_bytes_retry_loop(save_path: Path, data: bytes):
        while True:
            try:
                ...
                logger.info('Error while saving checkpoint, retrying in 1 minute: ', e)
                time.sleep(60)
    ...
    with (
        ...
        if vis_every > 0 and accelerator.is_main_process and initial_step == 0:
            ...
            for i_batch, batch in enumerate(tqdm(batches_for_vis, desc='Visualize GT', leave=False)):
                ...
                for i_instance in range(batch['image'].shape[0]):
                    ...
                    depth_in_i = depth_in[i_instance].numpy()
                    ...
                    mask_ = 1 - np.isnan(gt_depth_i).astype(depth_in_i.dtype)
                    depth_in_i = depth_in_i * mask_
                    cv2.imwrite(str(save_dir.joinpath(f'{idx:04d}/depth_in_vis.png')), cv2.cvtColor(colorize_depth(depth_in_i), cv2.COLOR_RGB2BGR))
        ...
        for i_step in range(initial_step, num_iterations):
            ...
            while i_accumulate < gradient_accumulation_steps:
                ...
                depth_in = batch['depth_in'].to(device)
                ...
                with accelerator.accumulate(model):
                    ...
                    with torch.autocast(device_type=accelerator.device.type, dtype=torch.bfloat16, enabled=enable_mixed_precision):
                        # output = model(image, num_tokens=num_tokens)
                        output = model(image, num_tokens=num_tokens, depth=depth_in)
                    ...
                    for i in range(current_batch_size):
                        ...
                        # if math.isinf(misc_dict['monitoring']['std']):
                        ...
                        for k, v in config['loss'][label_type[i]].items():
                            ...
                            if v['function'] == 'affine_invariant_global_loss':
                                ...
                                # loss_dict[k], misc_dict[k] = mask_bce_with_logits_loss(pred_mask_logits[i], gt_mask_fin[i], gt_mask_inf[i])
                            ...
                            elif v['function'] == 'depth_l1_loss':
                                loss_dict[k], misc_dict[k] = depth_l1_loss(pred_depth_reg[i], gt_depth[i], mask=gt_mask_fin[i], max_depth=100.0)
                            else:
                        ...
                        # print(f'gt_metric_scale = {gt_metric_scale}, max_gt_points = {gt_points[i].max()}, max_gt_normal = {gt_normal[i].max()}')
                        if torch.isnan(loss_).item():
                            pbar.write(f'NaN loss in process {accelerator.process_index}')
                            pbar.write(str(loss_dict))
                            logger.warning(f'NaN loss in process {accelerator.process_index}')
                    ...
                    if accelerator.sync_gradients:
                        ...
                        if any(torch.isnan(p.grad).any() for p in model.parameters() if p.grad is not None):
                            if accelerator.is_main_process:
                                ...
                                logger.warning('NaN gradients, skip update')
                            optimizer.zero_grad()
                            continue
                ...
                if accelerator.is_main_process:
                    ...
                    if enable_mlflow:
                        try:
                            ...
                            logger.info(f'Error while logging metrics to mlflow: {e}')
                    # record log info to logger
            ...
            if vis_every > 0 and accelerator.is_main_process and (i_step == initial_step or i_step % vis_every == 0):
                ...
                with torch.inference_mode():
                    for i_batch, batch in enumerate(tqdm(batches_for_vis, desc=f'Visualize: {i_step:08d}', leave=False)):
                        ...
                        output = unwrapped_model.infer(image, depth_in=depth_in, intrinsics=gt_intrinsics)
                        ...
                        for i_instance in range(image.shape[0]):
                            ...
                            if pred_points is not None:
                                ...
                                image_i = image[i_instance]
                                verts = pred_points_i[pred_mask_i][::4] if pred_mask_i is not None else pred_points_i.reshape(-1, 3)[::4]
                                verts_color = image_i[pred_mask_i][::4] if pred_mask_i is not None else image_i.reshape(-1, 3)[::4]
                                point_cloud = trimesh.PointCloud(verts, verts_color)
                                point_cloud.export(str(save_dir.joinpath(f'{idx:04d}/points.ply')))

...
def colorize_depth_metric(depth: np.ndarray, mask: np.ndarray = None, normalize: bool = True, cmap: str = 'Spectral', min_disp=None, max_disp=None, return_disp=True):
    if mask is None:
    ...
    if min_disp is not None:
        depth = np.where(depth <= 1 / min_disp, depth, np.nan)
    
    if max_disp is not None:
        depth = np.where(depth >= 1 / max_disp, depth, np.nan)

    disp = 1 / depth
    if normalize:
        # if min_disp is None or max_disp is None:
        #     # print('computing max/min disp')
        ...
        # min_disp, max_disp = 1/maxd, 1/mind
    ...
    if return_disp:
        return colored, min_disp, max_disp
    else:
        return colored

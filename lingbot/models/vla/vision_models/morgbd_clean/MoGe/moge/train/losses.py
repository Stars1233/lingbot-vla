...
def affine_invariant_global_loss(
    ...
    #    scale, shift = align_points_scale_z_shift(pred_points_lr.flatten(-3, -2), gt_points_lr.flatten(-3, -2), lr_mask.flatten(-2, -1), trunc=trunc)
    ...
    # Compute loss
    #     weight = (valid[..., None, None] & mask).float()
...
def affine_invariant_global_unW_loss(
    pred_points: torch.Tensor, 
    gt_points: torch.Tensor, 
    mask: torch.Tensor, 
    align_resolution: int = 64, 
...
def affine_invariant_local_unW_loss(
    pred_points: torch.Tensor, 
    gt_points: torch.Tensor, 
    gt_mask: torch.Tensor, 
    focal: torch.Tensor, 
    ...
    *batch_shape, height, width, _ = pred_points.shape
    ...
    # Align patch points
    (pred_patch_points_lr, gt_patch_points_lr), patch_lr_mask = mask_aware_nearest_resize((pred_patch_points, gt_patch_points), mask=patch_mask, size=(align_resolution, align_resolution))
    local_scale, local_shift = align_points_scale_xyz_shift(pred_patch_points_lr.flatten(-3, -2), gt_patch_points_lr.flatten(-3, -2), patch_lr_mask.flatten(-2), trunc=trunc)
    # local_scale, local_shift = align_points_scale_xyz_shift(pred_patch_points_lr.flatten(-3, -2), gt_patch_points_lr.flatten(-3, -2), patch_lr_mask.flatten(-2) / gt_patch_radius_3d[:, None].add(1e-7), trunc=trunc)
    if global_scale is not None:
        scale_differ = local_scale / global_scale[patch_batch_idx]
        patch_valid = (scale_differ > 0.1) & (scale_differ < 10.0) & (global_scale > 0)
    ...
    gt_mean = harmonic_mean(gt_points[..., 2], gt_mask, dim=(-2, -1))
    # patch_weight = patch_mask.float() / gt_patch_points[..., 2].clamp_min(0.1 * gt_mean[patch_batch_idx, None, None])          # [num_patches_nonempty, patch_h, patch_w]
    patch_weight = patch_mask.float()       # [num_patches_nonempty, patch_h, patch_w]
    loss = _smooth((pred_patch_points - gt_patch_points).abs() * patch_weight[..., None], beta=beta).mean(dim=(-3, -2, -1))    # [num_patches_nonempty]
...
def metric_scale_loss(pred_metric_scale: torch.Tensor, gt_metric_scale: torch.Tensor) -> torch.tensor:
    loss = (torch.log(pred_metric_scale) - torch.log(gt_metric_scale.detach()))**2
    return loss, {}

def img_rec_loss(img_rec: torch.Tensor, img_tgt: torch.Tensor) -> torch.Tensor:
    loss = (img_rec - img_tgt)**2
    loss = loss.mean()
    return loss, {}

def depth_loss(depth: torch.Tensor, gt_depth: torch.Tensor, mask: torch.Tensor, max_depth: float=30.0) -> torch.Tensor:
    mask_depth = mask & (gt_depth < max_depth)
    loss = (depth[mask_depth] - gt_depth[mask_depth])**2
    loss = loss.mean()
    return loss, {}

def points_global_loss(
    pred_points: torch.Tensor, 
    gt_points: torch.Tensor, 
    mask: torch.Tensor, 
    beta: float = 0.0, 
    ...
    # Compute loss
    valid = mask & (gt_points[..., 2] > min_depth) & (gt_points[..., 2] < max_depth)
    weight = valid.float()
    weight = weight.clamp_max(10.0 * weighted_mean(weight, mask, dim=(-2, -1), keepdim=True))   # In case your data contains extremely small depth values
    ...
    # Record any scalar metric
    misc = {
        'pcd_truncated_error': weighted_mean(err.clamp_max(1.0), mask).item(),
        'pcd_delta': weighted_mean((err < 1).float(), mask).item()
    }

    return loss, misc

def points_local_loss(
    pred_points: torch.Tensor, 
    gt_points: torch.Tensor, 
    gt_mask: torch.Tensor, 
    focal: torch.Tensor, 
    ...
    *batch_shape, height, width, _ = pred_points.shape
    batch_size = math.prod(batch_shape)
    pred_points, gt_points, gt_mask = pred_points.reshape(-1, height, width, 3), gt_points.reshape(-1, height, width, 3), gt_mask.reshape(-1, height, width)
    ...
    patch_mask = patch_mask & (gt_patch_points[..., 2] > min_depth) & (gt_patch_points[..., 2] < max_depth)
    patch_weight = patch_mask.float()       # [num_patches_nonempty, patch_h, patch_w]
    ...
    # Record any scalar metric
    misc = {
        'pcd_truncated_error': weighted_mean(err.clamp_max(1), patch_mask).item(),
        'pcd_delta': weighted_mean((err < 1).float(), patch_mask).item()
    }

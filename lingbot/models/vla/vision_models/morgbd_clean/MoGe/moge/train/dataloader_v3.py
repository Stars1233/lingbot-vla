...
class TrainDataLoaderPipeline:
    def __init__(self, config: dict, batch_size: int, num_load_workers: int = 4, num_process_workers: int = 8, buffer_size: int = 8):
        ...
        self.mask_depth_on = config.get('mask_depth_on', True)
        self.mask_max_ratio = config.get('mask_max_ratio', 0.2)
        self.mask_max_num = config.get('mask_max_num', 21)
        self.mask_min_num = config.get('mask_min_num', 20)
        self.min_mask_size_lb = config.get('min_mask_size_lb', 50)
        self.max_mask_size_lb = config.get('max_mask_size_lb', 200)
        self.noise_depth_on = config.get('noise_depth_on', False)
        assert self.mask_min_num < self.mask_max_num
    ...
    def _sample_batch(self):
        ...
        while True:
            ...
            for _ in range(self.batch_size):
                ...
                path = Path(self.datasets[dataset_name]['path'], filename.split('/')[-1])

                clamp_max_depth = self.datasets[dataset_name].get('clamp_max_depth', self.clamp_max_depth)
    ...
    def _load_instance(self, instance: dict):
        try:
            ...
            # get raw depth
            depth_raw, _ = read_depth(Path(instance['path'], self.datasets[instance['dataset']].get('depth_raw', 'depth.png')))
            
            clamp_max_depth = instance['clamp_max_depth']
            max_mask = depth >= clamp_max_depth
            max_mask_raw = depth_raw >= clamp_max_depth

            # set to nan
            depth[max_mask] = np.nan
            depth_raw[max_mask_raw] = np.nan
            ...
            # process raw depth
            depth_raw_mask = np.isfinite(depth_raw)
            depth_raw = np.nan_to_num(depth_raw, nan=1e-8, posinf=1e-8, neginf=1e-8)
    ...
    def _process_instance(self, instance: Dict[str, Union[np.ndarray, str, float, bool]]):
        ...
        depth_raw, depth_raw_mask = instance['depth_raw'], instance['depth_raw_mask']
        ...
        depth_raw_nearest = depth_raw[resize_index]
        edge_raw_mask = depth_occlusion_edge_numpy(depth_raw, mask=depth_raw_mask, thickness=2, tol=0.01)
        edge_raw_mask = edge_raw_mask[resize_index]
        distance_raw_nearest = norm3d(utils3d.numpy.depth_to_points(depth_raw_nearest, intrinsics=intrinsics))
        ...
        if self.depth_interpolation == 'bilinear':
            ...
            depth_raw_mask_bilinear = cv2.resize(depth_raw_mask.astype(np.float32), (rescaled_w, rescaled_h), interpolation=cv2.INTER_LINEAR)
            depth_raw_bilinear = 1 / cv2.resize(1 / (depth_raw+1e-10), (rescaled_w, rescaled_h), interpolation=cv2.INTER_LINEAR)
            distance_raw_bilinear = norm3d(utils3d.numpy.depth_to_points(depth_raw_bilinear, intrinsics=intrinsics))
        ...
        tgt_depth_raw_nearest = cv2.remap(distance_raw_nearest, pixel_remap[:, :, 0], pixel_remap[:, :, 1], cv2.INTER_NEAREST) / tgt_ray_length
        tgt_edge_raw_mask = cv2.remap(edge_raw_mask.astype(np.uint8), pixel_remap[:, :, 0], pixel_remap[:, :, 1], cv2.INTER_NEAREST) > 0
        ...
        if self.depth_interpolation == 'bilinear':
            ...
            tgt_depth_raw_mask_bilinear = cv2.remap(depth_raw_mask_bilinear, pixel_remap[:, :, 0], pixel_remap[:, :, 1], cv2.INTER_LINEAR)
            tgt_depth_raw_bilinear = cv2.remap(distance_raw_bilinear, pixel_remap[:, :, 0], pixel_remap[:, :, 1], cv2.INTER_LINEAR) / tgt_ray_length
            tgt_depth_raw = np.where((tgt_depth_raw_mask_bilinear == 1) & ~tgt_edge_raw_mask, tgt_depth_raw_bilinear, tgt_depth_raw_nearest)
        else:
            tgt_depth = tgt_depth_nearest
            tgt_depth_raw = tgt_depth_raw_nearest
        ...
        if tgt_depth_mask.sum() / tgt_depth_mask.size < 0.001:
            ...
            tgt_depth_raw = np.ones_like(tgt_depth_raw)
        ...
        if rng.choice([True, False]):
            ...
            tgt_depth_raw = np.flip(tgt_depth_raw, axis=1).copy()
        ...
        tgt_depth_raw = np.clip(tgt_depth_raw, 0, max_depth)
        tgt_depth_raw = np.nan_to_num(tgt_depth_raw, nan=0.0)

        if self.noise_depth_on:
            depth_noise = np.random.uniform(-0.015, 0.015, size=tgt_depth_raw.shape)
            depth_noise2 = np.random.normal(loc=0.0, scale=0.005, size=tgt_depth_raw.shape)
            final_noise = np.clip(depth_noise + depth_noise2, -0.1, 0.1)
            final_noise = final_noise * tgt_depth_mask.astype(dtype=final_noise.dtype)

            tgt_depth_raw = tgt_depth_raw + depth_noise + depth_noise2
            tgt_depth_raw = np.clip(tgt_depth_raw, 0.0, max_depth)

            # cv2.imwrite('mask.png', tgt_depth_mask * 255)
            # import pdb; pdb.set_trace()

        if self.mask_depth_on:
            num_blocks = np.random.randint(self.mask_min_num, self.mask_max_num)
            dh, dw = tgt_depth_raw.shape[0], tgt_depth_raw.shape[1]
            block_mask = np.ones((dh, dw), dtype=tgt_depth_raw.dtype)
            for _ in range(num_blocks):
                ...
                min_block_size_h = min(max(self.min_mask_size_lb, max_block_size_h*0.2), max_block_size_h-1)
                min_block_size_w = min(max(self.min_mask_size_lb, max_block_size_w*0.2), max_block_size_w-1)
            ...
            tgt_depth_raw = tgt_depth_raw * block_mask

        tgt_depth_raw = np.nan_to_num(tgt_depth_raw, nan=0.0)
        tgt_depth_raw[tgt_depth_raw > clamp_max_depth] = 0.0
        ...
        instance.update({
            ...
            'depth_raw': torch.from_numpy(tgt_depth_raw).float(),
    ...
    def _collate_batch(self, instances: List[Dict[str, Any]]):
        batch = {k: torch.stack([instance[k] for instance in instances], dim=0) for k in ['image', 'depth', 'depth_raw', 'depth_mask', 'depth_mask_fin', 'depth_mask_inf', 'intrinsics']}
        batch = {

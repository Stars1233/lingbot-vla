...
class MoGeModel(nn.Module):
    ...
    def forward(self, image: torch.Tensor, depth: torch.Tensor, num_tokens: Union[int, torch.LongTensor]) -> Dict[str, torch.Tensor]:
        ...
        # Heads decoding
        # points, normal, mask, img_rec = (getattr(self, head)(features)[-1] if hasattr(self, head) else None for head in ['points_head', 'normal_head', 'mask_head', 'img_head'])
        ...
        # Resize
        # points, normal, mask, img_rec = (F.interpolate(v, (img_h, img_w), mode='bilinear', align_corners=False, antialias=False) if v is not None else None for v in [points, normal, mask, img_rec])
        ...
        #     img_rec = F.normalize(img_rec)

        # img_tgt = (image - self.encoder.image_mean) / self.encoder.image_std

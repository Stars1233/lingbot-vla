...
from .modules_v3_dinov3 import DINOv3Encoder, MLP, ConvStack
...
class MoGeModel(nn.Module):
    ...
    def rgbd_encoder(self, image: torch.Tensor, depth: torch.Tensor, num_tokens: int=None, resolution_level: int = 9):
        ...
        # Backbones encoding
        features, cls_token, _, _  = self.encoder(image, depth, base_h, base_w, return_class_token=True)

        return features, cls_token

    def forward(self, image: torch.Tensor, depth: torch.Tensor, num_tokens: Union[int, torch.LongTensor]) -> Dict[str, torch.Tensor]:
        batch_size, _, img_h, img_w = image.shape
        device, dtype = image.device, image.dtype
        if depth.dim() == 3:
            depth = depth.unsqueeze(1)  # b, 1, h, w
        ...
        # Backbones encoding
        features, cls_token, _, _ = self.encoder(image, depth, base_h, base_w, return_class_token=True, remap_depth=self.remap_input_depth)
        ...
        # chunk_size = 32
        # features_chunk = []
        # for ci in range(0, batch_size, chunk_size):
        #     features_ci = self.neck([feat[ci:ci+chunk_size] for feat in features])
        #     features_chunk.append(features_ci)
        # features_ = []
        # for i in range(len(features_chunk)):
        #     features_[i] = torch.cat([feat[i] for feat in features_chunk], dim=0)
        # features = features_
    ...
    def infer(
        ...
        with torch.autocast(device_type=self.device.type, dtype=torch.float16, enabled=use_fp16 and self.dtype != torch.float16):
            output = self.forward(image, depth, num_tokens=num_tokens)

...
class DINOv3Encoder(nn.Module):
    ...
    def __init__(self, backbone: str, intermediate_layers: Union[int, List[int]], dim_out: int, ignore_layers: Union[str, List[str]]=[], in_chans: int=3, strict: bool=True, depth_mask_emb_mode='', mask_ratio=0.6, img_mask_ratio=0.0, **deprecated_kwargs):
        ...
        self.backbone = self.hub_loader(pretrained=False, in_depth_mask_on=True, depth_mask_emb_mode=depth_mask_emb_mode, mask_ratio=mask_ratio, img_mask_ratio=img_mask_ratio)
    ...
    def forward(self, image: torch.Tensor, depth: torch.Tensor, token_rows: Union[int, torch.LongTensor], token_cols: Union[int, torch.LongTensor], return_class_token: bool = False, remap_depth: str='linear'):
        ...
        depth_16 = F.interpolate(depth, (token_rows * 16, token_cols * 16), mode="nearest")
        # input_16 = torch.cat([image_16, depth_16], dim=1)

        dmask_16 = (depth_16 > 0.01).float()
        depth_16 = depth_16 * dmask_16
        ...
        input_16 = torch.cat([image_16, depth_16, dmask_16], dim=1)
        
        
        # Get intermediate layers from the backbone
        # features, mae_mask, mae_ids_restore = self.backbone.get_intermediate_layers_mae(input_16, n=self.intermediate_layers, return_class_token=True)
        features = self.backbone.get_intermediate_layers(input_16, n=self.intermediate_layers, return_class_token=True)

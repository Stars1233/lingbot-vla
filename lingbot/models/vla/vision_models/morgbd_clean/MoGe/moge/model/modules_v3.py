...
class DINOv2Encoder(nn.Module):
    ...
    def __init__(self, backbone: str, intermediate_layers: Union[int, List[int]], dim_out: int, ignore_layers: Union[str, List[str]]=[], in_chans: int=3, strict: bool=True, depth_mask_emb_mode='', mask_ratio=0.6, img_mask_ratio=0.0, **deprecated_kwargs):
        ...
        self.backbone = self.hub_loader(pretrained=False, in_chans=in_chans, in_depth_mask_on=True, depth_mask_emb_mode=depth_mask_emb_mode, mask_ratio=mask_ratio, img_mask_ratio=img_mask_ratio)
    ...
    def forward(self, image: torch.Tensor, depth: torch.Tensor, token_rows: Union[int, torch.LongTensor], token_cols: Union[int, torch.LongTensor], return_class_token: bool = False, remap_depth: str='linear', depth_down_scale: int=2):
        ...
        depth_14 = F.interpolate(depth, (token_rows //depth_down_scale * 14 , token_cols //depth_down_scale * 14 ), mode="nearest")
        
        dmask_14 = (depth_14 > 0.01).float()
        depth_14 = depth_14 * dmask_14

        if remap_depth == 'linear':
            pass
        elif remap_depth == 'log':
            mask = depth_14 == 0.0
        ...
        # input_14 = torch.cat([image_14, depth_14, dmask_14], dim=1)
        
        # Get intermediate layers from the backbone
        # features, mae_mask, mae_ids_restore = self.backbone.get_intermediate_layers_mae(input_14, n=self.intermediate_layers, return_class_token=True)
        features, mae_mask, mae_ids_restore = self.backbone.get_intermediate_layers_mae(image_14, 
        ...
        if mae_mask is not None:
            raise
            assert mae_ids_restore is not None
            features_ = []
            for (x, cls_token) in features:
                # append mask tokens to sequence
                mask_tokens = self.mask_token_mae.repeat(x.shape[0], mae_ids_restore.shape[1] + 1 - x.shape[1], 1)
                x_ = torch.cat([x, mask_tokens], dim=1)  # no cls token
                x_ = torch.gather(x_, dim=1, index=mae_ids_restore.unsqueeze(-1).repeat(1, 1, x.shape[2]))  # unshuffle
                features_.append((x_, cls_token))
            features = features_
        ...
        if return_class_token:
            return x, features[-1][1], mae_mask, mae_ids_restore
        else:

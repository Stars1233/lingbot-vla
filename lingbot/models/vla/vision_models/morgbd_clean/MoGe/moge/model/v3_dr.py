...
from .modules_v3 import DINOv2Encoder, MLP, ConvStack
...
class MoGeModel(nn.Module):
    ...
    def __init__(self, 
        ...
        remap_input_depth: Literal['linear', 'log'] = 'linear',
        num_tokens_range: List[int] = [1200, 3600],
        img_head: Dict[str, Any] = None,
        depth_head: Dict[str, Any] = None,
        conf_head: Dict[str, Any] = None,
        dr_mode: Literal['base', 'fuse_cls'] = 'base',
        cls_reduce_layer: Dict[str, Any] = None,
        **deprecated_kwargs
    ):
        ...
        self.remap_input_depth = remap_input_depth
        ...
        if cls_reduce_layer is not None:
            self.cls_reduce_layer = MLP(**cls_reduce_layer)
        
        if img_head is not None:
            self.img_head = ConvStack(**img_head)
        
        if depth_head is not None:
            self.depth_head = ConvStack(**depth_head)
        
        if conf_head is not None:
            self.conf_head = ConvStack(**conf_head)
    ...
    def rgbd_encoder(self, image: torch.Tensor, depth: torch.Tensor, num_tokens: int=None, resolution_level: int = 9):
        if image.dim() == 3:
            omit_batch_dim = True
        ...
        if depth.dim() == 3:
            ...
            depth = depth.unsqueeze(0).unsqueeze(0)  # b, 1, h, w
        assert depth.dim() == 4
        depth = depth.to(dtype=self.dtype, device=self.device)
        ...
        # Backbones encoding
        features, cls_token, mae_mask, mae_ids_restore  = self.encoder(image, depth, base_h, base_w, return_class_token=True)
        return features, cls_token

    def forward_feat(self, image: torch.Tensor, depth: torch.Tensor, num_tokens: Union[int, torch.LongTensor], depth_down_scale: int=1):
        batch_size, _, img_h, img_w = image.shape
        device, dtype = image.device, image.dtype
        if depth.dim() == 3:
            depth = depth.unsqueeze(1)  # b, 1, h, w
        ...
        # Backbones encoding
        features, cls_token, _, _  = self.encoder(image, depth, base_h, base_w, return_class_token=True, remap_depth=self.remap_input_depth, depth_down_scale=depth_down_scale)
        return features, cls_token

    def forward(self, image: torch.Tensor, depth: torch.Tensor, num_tokens: Union[int, torch.LongTensor], depth_down_scale: int=1) -> Dict[str, torch.Tensor]:
        batch_size, _, img_h, img_w = image.shape
        device, dtype = image.device, image.dtype
        if depth.dim() == 3:
            depth = depth.unsqueeze(1)  # b, 1, h, w
        ...
        # Backbones encoding
        features, cls_token, mae_mask, mae_ids_restore  = self.encoder(image, depth, base_h, base_w, return_class_token=True, remap_depth=self.remap_input_depth, depth_down_scale=depth_down_scale)
        ...
        # global_feat = self.cls_reduce_layer(cls_token)
        # global_feat = getattr(self, 'cls_reduce_layer')(cls_token) if hasattr(self, 'cls_reduce_layer') else cls_token
        # if self.dr_mode == 'fuse_cls_add':
        #     # fused_feat = features[0] + global_feat[..., None, None]
        #     # fused_features = [fused_feat] + features[1:]
        #     # fused_features = self.fused_neck(features)
        ...
        # Heads decoding
        points, normal, mask, depth_reg, conf = (getattr(self, head)(features)[-1] if hasattr(self, head) else None for head in ['points_head', 'normal_head', 'mask_head', 'depth_head', 'conf_head'])
        metric_scale = self.scale_head(cls_token) if hasattr(self, 'scale_head') else None
        
        # Resize
        points, normal, mask, depth_reg, conf = (F.interpolate(v, (img_h, img_w), mode='bilinear', align_corners=False, antialias=False) if v is not None else None for v in [points, normal, mask, depth_reg, conf])
        ...
        if depth_reg is not None:
            depth_reg = depth_reg.squeeze(1).exp()
        if conf is not None:
            conf = conf.squeeze(1).sigmoid()
    ...
    def infer(
        ...
        with torch.autocast(device_type=self.device.type, dtype=torch.float16, enabled=use_fp16 and self.dtype != torch.float16):
            output = self.forward(image, depth, num_tokens=num_tokens, depth_down_scale=depth_down_scale)
        points, normal, mask, metric_scale, depth_reg = (output.get(k, None) for k in ['points', 'normal', 'mask', 'metric_scale', 'depth_reg'])

        # Always process the output in fp32 precision
        points, normal, mask, metric_scale, fov_x, depth_reg = map(lambda x: x.float() if isinstance(x, torch.Tensor) else x, [points, normal, mask, metric_scale, fov_x, depth_reg])
        with torch.autocast(device_type=self.device.type, dtype=torch.float32):
            ...
            if apply_mask and mask_binary is not None:
                ...
                depth_reg = torch.where(mask_binary, depth_reg, torch.inf) if depth_reg is not None else None
    ...
    def infer_feat(
        ...
        # Forward pass
        with torch.autocast(device_type=self.device.type, dtype=torch.float16, enabled=use_fp16 and self.dtype != torch.float16):
            feat, cls_token = self.forward_feat(image, depth, num_tokens=num_tokens, depth_down_scale=depth_down_scale)

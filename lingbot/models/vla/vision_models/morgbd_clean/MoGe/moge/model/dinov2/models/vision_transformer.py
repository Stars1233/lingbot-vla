...
class DinoVisionTransformer(nn.Module):
    ...
    def prepare_tokens_with_masks_old(self, x, masks=None):
        B, nc, h, w = x.shape
        mae_mask, mae_ids_restore = None, None
        ...
        if self.in_depth_mask_on:
            ...
            if 'catFeat' in self.depth_mask_emb_mode:
                ...
                assert masks is None
                N, L, _ = x_depth_mask.shape  # batch, length, dim
                depth_pose_enc = self.interpolate_pos_encoding_without_cls(x_depth_mask, h, w, self.pos_embed[:, 1:])
                mask_ratio = self.mask_ratio
                if mask_ratio == 0:
                    x_depth_mask_masked = x_depth_mask
                    depth_pose_enc_masked = depth_pose_enc.repeat(N,1,1)
                else:
                    if B == 1 and not self.training:
                        ...
                        ids_keep = ids_shuffle[:, :len_keep]    
                        x_depth_mask_masked = torch.gather(x_depth_mask, dim=1, index=ids_keep.unsqueeze(-1).repeat(1, 1, x_depth_mask.shape[-1]))
                        depth_pose_enc_masked = torch.gather(depth_pose_enc.repeat(N,1,1), dim=1, index=ids_keep.unsqueeze(-1).repeat(1, 1, depth_pose_enc.shape[-1]))

                img_pose_enc = self.interpolate_pos_encoding_without_cls(x_img, h, w, self.pos_embed[:, 1:]).repeat(B, 1, 1)
                img_mask_ratio = self.img_mask_ratio
                if img_mask_ratio == 0:
                    x_img_masked = x_img
                    img_pose_enc_masked = img_pose_enc
                    ...
                    if B == 1 and not self.training:
                        ...
                        x_img_masked = torch.gather(x_img, dim=1, index=img_ids_keep.unsqueeze(-1).repeat(1, 1, x_img.shape[-1]))
                        img_pose_enc_masked = torch.gather(img_pose_enc.repeat(N,1,1), dim=1, index=img_ids_keep.unsqueeze(-1).repeat(1, 1, img_pose_enc.shape[-1]))
                ...
                # num_img_patches = x_img.shape[1]
                # cls_img_pos_enc = self.interpolate_pos_encoding(x[:, :num_img_patches+1], h, w)
                # cls_img_pos_enc = cls_img_pos_enc.repeat(B, 1, 1)
                # cls_img_depth_pos_enc = torch.cat([cls_img_pos_enc, depth_pose_enc_masked], dim=1)
                # type_enc = torch.ones(x.shape[0], x.shape[1], 1).to(device=x.device, dtype=x.dtype)
                # type_enc[:, 0] = 0 # cls token
                # type_enc[:,1+num_img_patches:] = 2 # depth token
                # x = x + cls_img_depth_pos_enc + type_enc
    ...
    def _get_intermediate_layers_not_chunked(self, x, n=1, return_mae_aux=False):
    ...
    def _get_intermediate_layers_chunked(self, x, n=1, return_mae_aux=False):
        x, mae_mask, mae_ids_restore = self.prepare_tokens_with_masks(x)
    ...
    def get_intermediate_layers_mae(
        ...
        if self.chunked_blocks:
            outputs, mae_mask, mae_ids_restore = self._get_intermediate_layers_chunked(x, n, return_mae_aux=True)
        else:
            outputs, mae_mask, mae_ids_restore = self._get_intermediate_layers_not_chunked(x, n, return_mae_aux=True)
        if norm:
            outputs = [self.norm(out) for out in outputs]
        class_tokens = [out[:, 0] for out in outputs]
        outputs = [out[:, 1 + self.num_register_tokens :] for out in outputs]

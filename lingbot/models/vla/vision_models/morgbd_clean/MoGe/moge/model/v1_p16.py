...
class MoGeModel(nn.Module):
    ...
    def __init__(self, 
        ...
        **deprecated_kwargs
        ...
        encoder_split = encoder.split('_')
        encoder = '_'.join(encoder_split[:2])
        hub_loader = getattr(importlib.import_module(".dinov2.hub.backbones", __package__), encoder)
        
        if len(encoder_split)==3:
            kwargs = {
                'img_size': int(encoder_split[-1]),
                'patch_size': 16,
            }
            self.backbone = hub_loader(pretrained=False,**kwargs)
        else:
            kwargs = {
                'img_size': 512, 
                'patch_size': 16,
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: Union[str, Path, IO[bytes]], model_kwargs: Optional[Dict[str, Any]] = None, **hf_kwargs) -> 'MoGeModel':
        ...
        if pretrained_model_name_or_path.startswith('oss://'):
            _temp_url = pretrained_model_name_or_path[6:].split('/')
            bucket = _temp_url[0]
            path = '/'.join(_temp_url[1:])
            import os, train_utils
            oss_manager = train_utils.lazy_oss(                
                bucket,
                verbose=False,
            )
            with oss_manager.open(path,'rb') as f:
    ...
    def forward(self, image: torch.Tensor, num_tokens: int) -> Dict[str, torch.Tensor]:
        ...
        image_14 = F.interpolate(image, (resized_height // 16 * 16, resized_width // 16 * 16), mode="bilinear", align_corners=False, antialias=True)

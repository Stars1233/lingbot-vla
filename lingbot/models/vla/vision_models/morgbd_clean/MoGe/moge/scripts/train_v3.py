...
if (_package_root := str(Path(__file__).absolute().parents[3])) not in sys.path:
    sys.path.insert(0, _package_root)
...
from MoGe.moge.train.dataloader_v3 import TrainDataLoaderPipeline
...
from moge.utils.vis import colorize_depth, colorize_normal, colorize_depth_metric
...
# from utils.oss_manager import create_oss_manager
from train_utils.oss_manager import create_oss_manager
...
def setup_logger(name, save_dir, out_file='log.txt', json_format=False, rank=0):
    ...
    if json_format:
        formatter = jsonlogger.JsonFormatter("%(asctime)s %(name)s %(levelname)s: %(message)s",json_indent=4)
    else:
...
@click.option('--oss', type=str, default=None, help='the OSS root for exp sync')
@click.option('--num_load_workers', type=int, default=8, help='number of load workers')
@click.option('--num_process_workers', type=int, default=8, help='number of process workers')
def main(
    config_path: str,
    workspace: str,
    checkpoint_path: str,
    ...
    accelerator = Accelerator(
        ...
        mixed_precision='bf16' if enable_mixed_precision else None,
        kwargs_handlers=[
    ...
    if accelerator.is_main_process:
        if enable_mlflow:
            try:
                ...
                logger.error('Failed to log config to MLFlow')
        Path(workspace).mkdir(parents=True, exist_ok=True)
        with Path(workspace).joinpath('config.json').open('w') as f:
            json.dump(config, f, indent=4)

    logger = setup_logger('moge', workspace, out_file='train.log', json_format=True, rank=accelerator.process_index)    
    ...
    logger.info(f'Total parameters: {count_total_parameters}')
    ...
    for i, count in enumerate(count_grouped_parameters):
        logger.info(f'- Group {i}: {count} parameters')

    if oss is not None and accelerator.is_main_process:
        assert oss.startswith('oss://')
        _split = oss[6:].split('/')
        ...
        if checkpoint_path is None:
            ...
            if checkpoint_path.exists():
                ...
                if 'model' not in checkpoint and (checkpoint_model_path := Path(workspace, 'checkpoint', f'{i_step:08d}.pt')).exists():
                    logger.info(f'Load model checkpoint: {checkpoint_model_path}')
                ...
                if 'optimizer' not in checkpoint and (checkpoint_optimizer_path := Path(workspace, 'checkpoint', f'{i_step:08d}_optimizer.pt')).exists():
                    logger.info(f'Load optimizer checkpoint: {checkpoint_optimizer_path}')
                ...
                if enable_ema and accelerator.is_main_process:
                    if 'ema_model' not in checkpoint and (checkpoint_ema_model_path := Path(workspace, 'checkpoint', f'{i_step:08d}_ema.pt')).exists():
                        logger.info(f'Load EMA model checkpoint: {checkpoint_ema_model_path}')
    ...
    with accelerator.local_main_process_first():
        train_data_pipe = TrainDataLoaderPipeline(
            ...
            buffer_size=batch_size_forward*10,
        )
        # train_data_pipe = TrainDataLoaderPipeline(
        #     config['data'], 
        #     batch_size_forward,
    ...
    with (
        ...
        if accelerator.is_main_process:
            ...
            if num_vis_images == 0:
                 num_vis_images = batch_size_forward
        ...
        if vis_every > 0 and accelerator.is_main_process and initial_step == 0:
            ...
            for i_batch, batch in enumerate(tqdm(batches_for_vis, desc='Visualize GT', leave=False)):
                ...
                for i_instance in range(batch['image'].shape[0]):
                    ...
                    gt_depth_raw_i = gt_depth_raw[i_instance].numpy()
                    gt_mask_depth_raw_i = gt_depth_raw_i > 0.001
                    ...
                    vis_depth_gt, min_disp, max_disp = colorize_depth_metric(gt_depth_i, gt_mask_i)
                    cv2.imwrite(str(save_dir.joinpath(f'{idx:04d}/depth_vis.png')), cv2.cvtColor(vis_depth_gt, cv2.COLOR_RGB2BGR))

                    vis_depth_raw, _, _ = colorize_depth_metric(gt_depth_raw_i, gt_mask_i, max_disp=max_disp)
                    cv2.imwrite(str(save_dir.joinpath(f'{idx:04d}/depth_raw_vis.png')), cv2.cvtColor(vis_depth_raw, cv2.COLOR_RGB2BGR))
                    
                    cv2.imwrite(str(save_dir.joinpath(f'{idx:04d}/mask_depth_raw.png')), gt_mask_depth_raw_i * 255)
                    ...
                    verts = gt_points_i[gt_mask_i][::2]
                    verts_color = image_i[gt_mask_i][::2]
                    point_cloud = trimesh.PointCloud(verts, verts_color)
                    point_cloud.export(str(save_dir.joinpath(f'{idx:04d}/points.ply')))

            if oss_manager:
                save_checkpoint_executor.submit(
                    oss_manager.upload_from_local,
                    str(save_dir),
                    str(oss_root/save_dir),
                )
        ...
        for i_step in range(initial_step, num_iterations):
            ...
            while i_accumulate < gradient_accumulation_steps:
                ...
                with accelerator.accumulate(model):
                    ...
                    with torch.autocast(device_type=accelerator.device.type, dtype=torch.float16, enabled=enable_mixed_precision):
                        output = model(image, depth_raw, num_tokens=num_tokens)
                    pred_points, pred_mask, pred_metric_scale = output.get('points', None), output.get('mask', None), output.get('metric_scale', None)
                    img_rec, img_tgt = output.get('img_rec', None), output.get('img_tgt', None)
                    pred_depth_reg = output.get('depth_reg', None)
                    pred_points_from_depth_reg = utils3d.torch.depth_to_points(pred_depth_reg, intrinsics=gt_intrinsics) if pred_depth_reg is not None else None
                    ...
                    for i in range(current_batch_size):
                        ...
                        for k, v in config['loss'][label_type[i]].items():
                            ...
                            if v['function'] == 'affine_invariant_global_loss':
                                ...
                                assert gt_metric_scale is not None
                                if gt_metric_scale == 0.:
                                    loss_dict[k], misc_dict[k] = torch.tensor([0.0]).cuda(), {}
                                else:
                                    loss_dict[k], misc_dict[k] = metric_scale_loss(pred_metric_scale[i], gt_metric_scale)
                            elif v['function'] == 'img_rec_loss':
                                assert img_rec is not None
                                loss_dict[k], misc_dict[k] = img_rec_loss(img_rec[i], img_tgt[i])
                            elif v['function'] == 'depth_loss':
                                loss_dict[k], misc_dict[k] = depth_loss(pred_depth_reg[i], gt_depth[i], gt_mask[i])
                            elif v['function'] == 'conf_loss':
                                pass
                            elif v['function'] == 'points_global_loss':
                                assert pred_points_from_depth_reg is not None
                                loss_dict[k], misc_dict[k] = points_global_loss(pred_points_from_depth_reg[i], gt_points[i], gt_mask[i])
                            elif v['function'] == 'points_local_loss':
                                assert pred_points_from_depth_reg is not None
                                loss_dict[k], misc_dict[k] = points_local_loss(pred_points_from_depth_reg[i], gt_points[i], gt_mask[i], gt_focal[i], **v['params'])
                            else:
                        ...
                        if torch.isnan(loss_).item():
                            logger.error(f'NaN loss in process {accelerator.process_index}')
                            logger.error(str(loss_dict))
                            # import pdb; pdb.set_trace()
                    ...
                    if accelerator.sync_gradients:
                        if not enable_mixed_precision and any(torch.isnan(p.grad).any() for p in model.parameters() if p.grad is not None):
                            if accelerator.is_main_process:
                                logger.error(f'NaN gradients, skip update')
                            optimizer.zero_grad()
                            continue
                ...
                if accelerator.is_main_process:
                    ...
                    if enable_mlflow:
                        try:
                            ...
                            logger.error(f'Error while logging metrics to mlflow: {e}')
                    record_dict = {f'loss.{key}': value for key, value in records.items()}
                    if isinstance(lr_scheduler.get_last_lr(), list):
                        for i, lr in enumerate(lr_scheduler.get_last_lr()):
                            record_dict[f'lr.{i:02d}'] = lr
                    else:
                        record_dict['lr'] = lr_scheduler.get_last_lr()
                    record_dict['step'] = i_step
                    record_dict['num_iterations'] = num_iterations
                    record_dict['batch_size'] = current_batch_size if current_batch_size is not None else -1
                    logger.info(msg='loss', extra=record_dict)
                    
                    if oss_manager:
                        save_checkpoint_executor.submit(
                            oss_manager.upload_from_local,
                            str(Path(workspace,'train.log')),
                            str(Path(oss_root, workspace, 'train.log'))
                        )
                records = []
            ...
            if accelerator.is_main_process and (i_step % save_every == 0) and (i_step > 0):
                ...
                logger.info(f'Save checkpoint: {i_step:08d}')
                ...
                if oss_manager:
                    save_checkpoint_executor.submit(
                        oss_manager.save_dict, state_dict, str(Path(oss_root, workspace, 'checkpoint', f'{i_step:08d}.pt'))
                    )
                ...
                if oss_manager:
                    save_checkpoint_executor.submit(
                        oss_manager.save_dict, state_dict, str(Path(oss_root, workspace, 'checkpoint', f'{i_step:08d}_optimizer.pt'))
                    )
                ...
                if enable_ema:
                    if oss_manager:
                        save_checkpoint_executor.submit(
                            oss_manager.save_dict, 
                            {
                                'model_config': config['model'],
                                'model': ema_model.module.state_dict(),
                            },
                            str(Path(oss_root, workspace, 'checkpoint', f'{i_step:08d}_ema.pt'))
                        )
                ...
                if oss_manager:
                    save_checkpoint_executor.submit(
                        ...
                        str(Path(oss_root,workspace, 'checkpoint', 'latest.pt'))
                    )                
                with io.BytesIO() as f:
                    torch.save({
                        'model_config': config['model'],
            ...
            if vis_every > 0 and accelerator.is_main_process and (i_step == initial_step or i_step % vis_every == 0):
                ...
                with torch.inference_mode():
                    for i_batch, batch in enumerate(tqdm(batches_for_vis, desc=f'Visualize: {i_step:08d}', leave=False)):
                        ...
                        image, gt_depth, gt_mask, gt_intrinsics = image.to(device)[:max_vis_num], gt_depth.to(device)[:max_vis_num], gt_mask.to(device)[:max_vis_num], gt_intrinsics.to(device)[:max_vis_num]
                        depth_raw = batch['depth_raw'].to(device)[:max_vis_num]

                        output = unwrapped_model.infer(image, depth_raw)
                        
                        pred_points, pred_depth, pred_mask = output.get('points', None), output.get('depth', None), output.get('mask', None)
                        pred_points = pred_points.cpu().numpy() if pred_points is not None else None
                        pred_depth = pred_depth.cpu().numpy() if pred_depth is not None else None
                        pred_mask = pred_mask.cpu().numpy() if pred_mask is not None else None

                        pred_depth_reg = output['depth_reg'] if 'depth_reg' in output else None
                        pred_points_from_depth_reg = utils3d.torch.depth_to_points(pred_depth_reg, intrinsics=gt_intrinsics) if pred_depth_reg is not None else None

                        pred_depth_reg = pred_depth_reg.cpu().numpy() if pred_depth_reg is not None else None
                        pred_points_from_depth_reg = pred_points_from_depth_reg.cpu().numpy() if pred_points_from_depth_reg is not None else None
                        ...
                        for i_instance in range(image.shape[0]):
                            ...
                            gt_depth_i = gt_depth[i_instance].cpu().numpy()
                            vis_depth_gt, min_disp, max_disp = colorize_depth_metric(gt_depth_i, gt_mask_i)
                            depth_raw_i = depth_raw[i_instance].cpu().numpy()
                            min_disp, max_disp = None, None
                            cv2.imwrite(str(save_dir.joinpath(f'{idx:04d}/depth_raw_vis.png')), cv2.cvtColor(colorize_depth_metric(depth_raw_i, gt_mask_i, max_disp=max_disp, return_disp=False), cv2.COLOR_RGB2BGR))

                            if pred_depth is not None:
                                pred_depth_i = pred_depth[i_instance]
                                cv2.imwrite(str(save_dir.joinpath(f'{idx:04d}/depth_vis.png')), cv2.cvtColor(colorize_depth_metric(pred_depth_i, pred_mask_i, return_disp=False), cv2.COLOR_RGB2BGR))
                            
                            if pred_depth_reg is not None:
                                pred_depth_reg_i = pred_depth_reg[i_instance]
                                cv2.imwrite(str(save_dir.joinpath(f'{idx:04d}/depth_reg_vis.png')), cv2.cvtColor(colorize_depth_metric(pred_depth_reg_i, pred_mask_i, max_disp=max_disp, return_disp=False), cv2.COLOR_RGB2BGR))

                            if pred_points is not None:
                                pred_points_i = pred_points[i_instance]
                                verts = pred_points_i[pred_mask_i][::2]
                                verts_color = image_i[pred_mask_i][::2]
                                point_cloud = trimesh.PointCloud(verts, verts_color)
                                point_cloud.export(str(save_dir.joinpath(f'{idx:04d}/points.ply')))

                            if pred_points_from_depth_reg is not None:
                                pred_points_from_depth_reg_i = pred_points_from_depth_reg[i_instance]
                                verts = pred_points_from_depth_reg_i[pred_mask_i][::2]
                                verts_color = image_i[pred_mask_i][::2]
                                point_cloud = trimesh.PointCloud(verts, verts_color)
                                point_cloud.export(str(save_dir.joinpath(f'{idx:04d}/points_from_depth_reg.ply')))

...
class DepthAnythingV2_Backbone(nn.Module):
    def __init__(
        self, 
        encoder='vitl', 
        features=256, 
        ...
        super(DepthAnythingV2_Backbone, self).__init__()
    ...
    @torch.no_grad()
    def infer_image(self, raw_image, input_size=336, device='cuda'):
        image = self.image2tensor(raw_image, input_size, device=device)
        features = self.forward(image)
        return features
    
    def image2tensor(self, raw_image, input_size=518, device='cuda'):        
        image = raw_image[:, [2, 1, 0], :, :] / 255.0
...
class DepthAnythingV2_Head(nn.Module):
    def __init__(
        self, 
        encoder='vitl', 
        features=256, 
        ...
        super(DepthAnythingV2_Head, self).__init__()

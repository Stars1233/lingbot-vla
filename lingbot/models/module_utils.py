...
def load_model_weights(
    ...
    for state_dict_iterator in tqdm(
        ...
        if 'pi0' in weights_path and len(parameter_names) != 0 and not load_vlm_only and not post_training and not incremental_training:
            embed_tokens = torch.load('/robby/share/MM/pi0torch_cache/vlm_embedding.pt')
            _dispatch_parameter(model, 'model.paligemma_with_expert.paligemma.language_model.model.embed_tokens.weight', embed_tokens, dtensor_factory)
            parameter_names.remove('model.paligemma_with_expert.paligemma.language_model.model.embed_tokens.weight')
